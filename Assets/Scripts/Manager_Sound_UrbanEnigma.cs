...
public class Manager_Sound_UrbanEnigma : MonoBehaviour
{
    [SerializeField] private AudioMixer mixer_UrbanEnigma;
    ...
    public void SetVolumeMusic_UrbanEnigma(float volume)
    {
        mixer_UrbanEnigma.SetFloat("Music", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
    ...
    public void SetVolumeSFX_UrbanEnigma(float volume)
    {
        mixer_UrbanEnigma.SetFloat("Sound", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
